...
- **결과:** 한 레포 한 배포. 나중에 Docling/임베딩/LLM은 HTTP로 붙인다.
...
### ADR-3. 저장소 = 인메모리 (기동 시 인제스천으로 구축)
- **결정:** `lib/store.ts` 가 모듈 로드 시 `ingestAll()`(data/sources 파싱) 결과로 인덱스를 만든다.
  파싱 실패/공백 시 `lib/seed.ts` 폴백. (SQLite/better-sqlite3 는 도입하지 않음 — 규모가 작아 불필요.)
- **왜:** 무설치·무상태·컨테이너 기동 시 재구축 → 볼륨/DB 없이 어떤 클라우드든 동일 동작.
- **확장:** PostgreSQL + pgvector (`lib/store.ts` 구현만 교체, 시그니처 유지).

### ADR-4. 검색 = 키워드 + 그래프 스코어링 (임베딩 아님)
- **결정:** MVP는 신경망 임베딩 없이 텍스트 매칭 + 그래프 근접도로 스코어링.
- **왜:** 임베딩 모델/벡터DB는 MVP 무게↑. 슬롯만 비워둔다.
- **확장:** 임베딩 API 호출 → 벡터DB(pgvector→Qdrant/Milvus, 과제② K8s VectorDB).

### ADR-5. 자연어 검색 = 규칙기반 엔티티 링크 (LLM/임베딩 아님, 기본값)
- **결정:** 자연어 질문은 `lib/nlsearch.ts` 의 **규칙기반**(지역→법규 매핑·유형 의도·동의어·그래프 확장)으로 처리.
- **맥락(측정):** 사내 자원을 실제로 시험했다.
...
| 저장소 | 인메모리(기동 시 `ingestAll()`, 실패 시 seed 폴백) | Postgres + pgvector |
| 검색 | 키워드 + 그래프 스코어링 | 임베딩 API → 벡터DB |
| 자연어 검색 | 규칙기반 엔티티 링크(`lib/nlsearch.ts`) | 사내 LLM(`NL_USE_LLM=1`) / 임베딩 |
...
| 배포 | `output:'standalone'` + Docker → **FEDA K8s(v2, 배포됨)** | HPA |
| 인증 | 없음 | OIDC/SSO |
...
- **실제 배포(v2):** FEDA K8s(Rocky 9.7, v1.30.14, containerd). 레지스트리 **`192.168.0.100:5000`**

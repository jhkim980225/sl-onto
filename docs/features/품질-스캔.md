...
스캔은 "무엇을 정리할지 찾기"만 한다 — 실행은 기존 `POST /api/curate`(병합·삭제) 경유, 사람 승인
필수 → 실행 계약 상세는 [큐레이션.md](큐레이션.md). 형식 온톨로지(메타모델) 위반 4종은 별도
스캐너([스키마-검증.md](스키마-검증.md))가 찾아 이 스캔 배열에 합류한다.

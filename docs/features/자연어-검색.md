...
## 책임
자연어 질문(한국어)을 좁고 통제된 온톨로지에 대한 **엔티티 링크 + 의도 파악 + 그래프 확장**으로 풀어
관련 객체를 관련도 순으로 반환한다. 키워드 검색([키워드-검색.md](키워드-검색.md))을 **보완**한다.
...
- **캔버스 스코프**: `semanticSearch`·`nodesMissingEmbedding` 은 `WHERE canvas_id = currentCanvas()`
  로 걸린다 — 벡터 후보도 현재 캔버스 안에서만 나온다.

### 임베딩 현황 (과장 금지)
- 임베딩 단위는 **노드 1개 = 벡터 1개**(`embedText(label, props)`). **문서 청킹은 미구현** —
  설계만 있다([document-chunking](../superpowers/specs/2026-07-20-document-chunking-design.md)).
- 모델은 여전히 **MiniLM-L12(384-dim, max 128 토큰)**. e5 계열 교체·긴 텍스트 청킹은 계획일 뿐
  코드에 없다. 짧은 라벨 위주라 128 토큰 한계가 아직 문제되지 않는다는 것이 현재 근거다.

...
> **아카이브.** 개발 완료 내역 스냅샷(1차 MVP, FMEA 중심 서술) — 제품이 도메인-비특정 벡터 RAG +
> 그래프 온톨로지로 피벗한 이후 현행 서술이 아니다. 현행 제품 문서는 [../README.md](../README.md) 참조.
...
- 실제 원천 파일 **40개**(xlsx FMEA 시트 · pptx 재발방지/8D · docx 품질리포트 · xlsx 참조 마스터 · dxf 도면)를
  기동 시 파싱해 온톨로지 구축: **180 노드 / 2,199 엣지 / 문서 41** (실패 시 seed 폴백 ≈275 노드).
...
### 3.4 그래프 추론 → 체크리스트 ([features/legacy-fmea/그래프-추론.md](../features/legacy-fmea/그래프-추론.md))
- `POST /api/infer`: 신규 설계 조건 → 그래프 탐색 → **계산된** 체크리스트 상위 8
  (제목 · 인과 설명 · 근거칩 · 확신도% · 실존 엣지 trace).
- 조건 반응: 북미→FMVSS 108 승격, 슬림→수축 항목 부스트 등.

### 3.5 FMEA 초안 생성 ([features/legacy-fmea/FMEA-초안생성.md](../features/legacy-fmea/FMEA-초안생성.md))
...
### 3.7 결로·습기 지역별 시나리오 ([features/legacy-fmea/결로-시나리오.md](../features/legacy-fmea/결로-시나리오.md))
...
- **캔버스 = 도메인(부서·제품군)별 완전 격리 워크스페이스** — 데이터도 스키마도 0에서 시작.
  기존 램프 데이터(180 노드 / 2,199 엣지 / 41 문서)는 `default` 캔버스로 귀속.

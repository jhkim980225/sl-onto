...
### 3.4 그래프 추론 → 체크리스트 ([features/그래프-추론.md](features/legacy-fmea/그래프-추론.md))
- `POST /api/infer`: 신규 설계 조건 → 그래프 탐색 → **계산된** 체크리스트 상위 8
  (제목 · 인과 설명 · 근거칩 · 확신도% · 실존 엣지 trace).
- 조건 반응: 북미→FMVSS 108 승격, 슬림→수축 항목 부스트 등.

### 3.5 FMEA 초안 생성 ([features/FMEA-초안생성.md](features/legacy-fmea/FMEA-초안생성.md))
...
### 3.7 결로·습기 지역별 시나리오 ([features/결로-시나리오.md](features/legacy-fmea/결로-시나리오.md))

// Python 사이드카(/embed) 클라이언트 — 상태 없음. 질의·노드 텍스트 → 384dim 벡터.
...
export async function embed(texts: string[]): Promise<number[][]> {
  if (!texts.length || !embedEnabled()) return [];
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), EMBED_TIMEOUT_MS);
  try {
    const res = await fetch(`${pysvc()}/embed`, {
      method: "POST",
    ...
    if (!res.ok) {
      console.warn(`[embed] pyservice ${res.status} — 임베딩 없이 진행`);
      return [];
    }
    const data = await res.json();
    return Array.isArray(data?.vectors) ? (data.vectors as number[][]) : [];
  } catch (e) {
    console.warn(`[embed] pyservice unavailable (${(e as Error).message}) — 임베딩 없이 진행`);
    return [];
  } finally {
    clearTimeout(timer);
  }
